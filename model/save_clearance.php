    ...
    if ($statusData['status'] === 'on hold' || $statusData['status'] === 'claimed' || $statusData['status'] === 'rejected') {
        ...
        if ($checkApprovedData['status'] !== 'approved') {
            ...
            if ($checkDuplicateData['count'] < 1) {
                $insert_query = "INSERT INTO tblclearance(`fullname`,`cert_name`,`purok`, `taxno`, `requirement`,`requester`,`email`,`quantity`) VALUES ('$fullname', '$cert_name', '$purok', '$tax', '$req','$fname', '$user_email', '$qty')";
                ...
                if ($result_resident === true) {
                    ...
                    if ($result_requested === true) {
                        $_SESSION['message'] = 'You have requested a certificate of clearance that has been sent!';
            ...
            $_SESSION['message'] = 'You have reached the maximum request of certificates of clearance. Please double-check your certificates status or go to the Barangay Office for clarification.';
            $_SESSION['success'] = 'info';
            header("Location: " . $_SERVER["HTTP_REFERER"]);
            exit();
        }
...
if ($matchCheckData['match_count'] === 0) {
    $_SESSION['message'] = 'Your account has not been fully registered. Please register first your account in here.';
    $_SESSION['success'] = 'danger';
    header("Location: ../resident_profiling.php");
    exit();
}

// Insert data into tblclearance
$insert_query = "INSERT INTO tblclearance(`fullname`,`cert_name`,`purok`, `taxno`, `requirement`,`requester`,`email`,`quantity`) VALUES ('$fullname', '$cert_name', '$purok', '$tax', '$req','$fname', '$user_email', '$qty')";
$result_resident = $conn->query($insert_query);
