...
if (!empty($fullname) && !empty($purok) && !empty($age)) {
    ...
    if ($statusCheckResult->num_rows > 0) {
        ...
        if ($statusData['status'] === 'on hold' || $statusData['status'] === 'claimed' || $statusData['status'] === 'rejected') {
            ...
            if ($checkApprovedData['status'] !== 'approved') {
                ...
                if ($checkDuplicateData['count'] < 1) {
                    $insert_query = "INSERT INTO tblfirstjob(`fullname`, `purok`, `age`, `cert_name`, `requester`, `email`, `requirement`)
                    ...
                    if ($result_resident === true) {
                        ...
                        if ($result_requested === true) {
                            $_SESSION['message'] = 'You have requested a certificate of first-time jobseekers that has been sent!';
                    ...
                    if ($result_requested === true) {
                        $_SESSION['message'] = 'You have requested a certificate of first-time jobseekers with the same requirement successfully!';
                        $_SESSION['success'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                        $_SESSION['success'] = 'danger';
                ...
                $_SESSION['message'] = 'You have reached the maximum request of certificates of first-time jobseekers. Please double-check your certificates status or go to the Barangay Office for clarification.';
                $_SESSION['success'] = 'info';
                header("Location: " . $_SERVER["HTTP_REFERER"]);
                exit();
            }
        ...
        if ($result_resident === true) {
            ...
            if ($result_requested === true) {
                $_SESSION['message'] = 'You have requested a certificate of first-time jobseekers successfully!';
                $_SESSION['success'] = 'success';
            } else {
                $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                $_SESSION['success'] = 'danger';
