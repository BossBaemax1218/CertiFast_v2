    ...
    if ($statusCheckResult->num_rows > 0) {
        ...
        if ($statusData['status'] === 'on hold' || $statusData['status'] === 'claimed' || $statusData['status'] === 'rejected') {
            ...
            if ($checkApprovedData['status'] !== 'approved') {
                ...
                if ($checkDuplicateData['count'] < 1) {
                    ...
                    if ($result_resident === true) {
                        ...
                        if ($result_requested === true) {
                            $_SESSION['message'] = 'You have requested a certificate of oath taking that has been sent!';
                        ...
                        $_SESSION['message'] = 'Something went wrong while inserting into tbloath: ' . $conn->error;
                        $_SESSION['success'] = 'danger';
                    }
                } else {
                    ...
                    if ($result_requested === true) {
                        $_SESSION['message'] = 'You have requested a certificate of oath taking with the same requirement successfully!';
                        $_SESSION['success'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                        $_SESSION['success'] = 'danger';
                ...
                $_SESSION['message'] = 'You have reached the maximum request of certificates of oath taking. Please double-check your certificates status or go to the Barangay Office for clarification.';
                $_SESSION['success'] = 'info';
                header("Location: " . $_SERVER["HTTP_REFERER"]);
                exit();
            }
        ...
        if ($result_resident === true) {
            ...
            if ($result_requested === true) {
                $_SESSION['message'] = 'You have requested a certificate of oath taking successfully!';
                $_SESSION['success'] = 'success';
            } else {
                $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                $_SESSION['success'] = 'danger';
