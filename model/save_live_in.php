    ...
    if ($statusCheckResult->num_rows > 0) {
        ...
        if ($statusData['status'] === 'on hold' || $statusData['status'] === 'claimed' || $statusData['status'] === 'rejected') {
            ...
            if ($checkApprovedData['status'] !== 'approved') {
                ...
                if ($checkDuplicateData['count'] < 1) {
                    ...
                    if ($result_resident === true) {
                        ...
                        if ($result_requested === true) {
                            $_SESSION['message'] = 'You have requested a certificate of live-in that has been sent!';
                    ...
                    $insert_requested = "INSERT INTO tblresident_requested(`resident_name`, `certificate_name`, `email`, `purok`, `status`)
                                        VALUES ('$fname', '$cert_name', '$user_email', '$purok', 'on hold')";
                    $result_requested = $conn->query($insert_requested);

                    if ($result_requested === true) {
                        $_SESSION['message'] = 'You have requested a certificate of live-in with the same requirement successfully!';
                        $_SESSION['success'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                        $_SESSION['success'] = 'danger';
                ...
                $_SESSION['message'] = 'You have reached the maximum request of certificates of live-in. Please double-check your certificates status or go to the Barangay Office for clarification.';
                $_SESSION['success'] = 'info';
                header("Location: " . $_SERVER["HTTP_REFERER"]);
                exit();
            }
        ...
        if ($result_resident === true) {
            ...
            if ($result_requested === true) {
                $_SESSION['message'] = 'You have requested a certificate of live-in successfully!';
                $_SESSION['success'] = 'success';
            } else {
                $_SESSION['message'] = 'Something went wrong while inserting into tblresident_requested: ' . $conn->error;
                $_SESSION['success'] = 'danger';
