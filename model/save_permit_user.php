<?php 
    ...
    if(!empty($business_name) && !empty($owner1) && !empty($address) && !empty($email) && !empty($location) && !empty($applied) && !empty($req)){
        ...
        $insert  = "INSERT INTO tblpermit (`business_name`, `owner1`, `email`, address, location, applied, status, cert_name, requester, req_email, requirement) VALUES ('$business_name', '$owner1','$email', '$address', '$location','$applied', 'on hold','$cert_name','$fname','$user_email','$req')";
