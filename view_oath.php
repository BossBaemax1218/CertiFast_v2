...
<?php 
   ...
   $oathid = $_GET['id'];
   $oathquery = "SELECT * FROM tbloath AS c JOIN tblresident_requested AS r ON c.requirement = r.requirement WHERE c.oath_id='$oathid' AND c.cert_name = r.certificate_name AND c.email = r.email AND c.requester = r.resident_name AND r.status IN ('on hold','approved')";
