...
<?php 
   ...
   $cleid = $_GET['id'];
   $clequery = "SELECT c.* FROM tblclearance AS c JOIN tblresident_requested AS r ON c.requirement = r.requirement WHERE c.c_id='$cleid' AND c.cert_name = r.certificate_name AND c.email = r.email AND c.requester = r.resident_name AND r.status IN ('on hold','approved')";
