<?php 
    ...
    $iid = $_GET['id'];
    $queries = "SELECT c.* FROM tblindigency AS c JOIN tblresident_requested AS r ON c.requirements = r.requirement WHERE c.indi_id='$iid' AND c.cert_name = r.certificate_name AND c.email = r.email AND c.requester = r.resident_name AND r.status IN ('on hold','approved')";
