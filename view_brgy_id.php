...
<?php 
   ...
   $brgyid = $_GET['id'];
   $brgyquery = "SELECT * FROM tblbrgy_id AS c JOIN tblresident_requested AS r ON c.requirement = r.requirement WHERE c.brgy_id='$brgyid' AND c.cert_name = r.certificate_name AND c.email = r.email AND c.requester = r.resident_name AND r.status IN ('on hold','approved')";
